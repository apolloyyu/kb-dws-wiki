---
source_path: "skills/mono/references/products/sheet/sheet-workbook.md"
...
本命令会创建表格、写入初始数据并回读校验，还可选择应用样式。**只要一个空表格请用 `dws sheet create`**；本命令必须给数据。

**写入初始数据**（`--values` 与 `--sheets` **二选一，必须给一个**）：

- `--values`：二维 JSON 数组，裸值写入默认工作表 A1 起。适合单表快速建表，无表头/类型语义，并自动识别数字/布尔。单元格只能是字符串/数字/布尔/null；上限 30000 单元格、编码后 2000000 字符。
- `--sheets`：typed table 数组，一次创建多个带数据的工作表。每项形如
