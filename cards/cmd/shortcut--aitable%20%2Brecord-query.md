...
description: Base ID
source: internal/shortcut/aitable/aitable.go:688
visible_flags: 9
...
- --filters <String>: 结构化过滤条件 JSON（可选）；先用 field get 完整读一遍表头，确定用户条件对应的字段和类型后再传值。人员、部门、群组禁止原值透传，必须分别经 aisearch person、contact +resolve-dept、chat
