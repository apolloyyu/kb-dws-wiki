---
source_path: "skills/mono/references/products/html.md"
...
`html` 面向钉盘或文档空间中的原生 `.html` / `.htm` 文件，把内容作为单个纯文本文件读写。在线富文本文档（`adoc`）仍使用 [`doc.md`](./doc.md)；原生 `.md` 文件使用 [`markdown.md`](./markdown.md)。
...
> 历史版本列表、下载指定版本、回滚与文件评论使用 [`drive.md`](./drive.md)。
...
- 原生 `.html` / `.htm` 内容读写用 `html`；在线富文本文档用 `doc`。
- 任意类型文件的一般上传/下载用 [`drive.md`](./drive.md)；明确需要 HTML 文件类型契约（扩展名校验、`text/html` MIME）时用 `html`。
